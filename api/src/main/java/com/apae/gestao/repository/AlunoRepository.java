...
public interface AlunoRepository extends JpaRepository<Aluno, Long> {
    ...
    @Query(
            ...
            WHERE (:nome IS NULL OR TRIM(:nome) = '' OR LOWER(a.nome) LIKE LOWER(CONCAT('%', TRIM(:nome), '%')))
            GROUP BY a.id, a.nome, a.nomeResponsavel, a.deficiencia, a.dataNascimento, a.telefoneResponsavel
            """,
            countQuery = "SELECT COUNT(DISTINCT a) FROM Aluno a WHERE (:nome IS NULL OR TRIM(:nome) = '' OR LOWER(a.nome) LIKE LOWER(CONCAT('%', TRIM(:nome), '%')))"
    )
    ...
    @Query(value = """
                ...
                WHERE (:nome IS NULL OR TRIM(:nome) = '' OR LOWER(a.nome) LIKE LOWER(CONCAT('%', TRIM(:nome), '%')))
                GROUP BY a.id, a.nome, a.nomeResponsavel, a.deficiencia, a.dataNascimento, a.telefoneResponsavel
            """, countQuery = "SELECT COUNT(DISTINCT a) FROM Aluno a JOIN a.turmaAlunos ta ON ta.isAlunoAtivo = true WHERE (:nome IS NULL OR TRIM(:nome) = '' OR LOWER(a.nome) LIKE LOWER(CONCAT('%', TRIM(:nome), '%')))")
    ...
    @Query(
            ...
            """,
            countQuery = "SELECT COUNT(DISTINCT a) FROM Aluno a JOIN a.turmaAlunos ta JOIN ta.turma t WHERE t.id = :turmaId AND ta.isAlunoAtivo = true"
    )
