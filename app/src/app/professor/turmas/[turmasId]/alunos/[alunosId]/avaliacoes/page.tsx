...
import { ArrowLeft, FileText, UserCircle, Plus, Edit, Trash2 } from "lucide-react";
...
export default function AvaliacoesAlunoPage() {
  ...
  const [avaliacoes, setAvaliacoes] = useState<Avaliacao[]>([
    {
      ...
      descricao: "O aluno demonstrou evolução motora significativa durante as atividades de coordenação.",
    },
    {
      id: 2,
      data: new Date(2025, 10, 10),
      descricao: "Escreveu o nome completo pela primeira vez com auxílio mínimo.",
    },
    {
      id: 3,
      data: new Date(2025, 10, 15),
      descricao: "Apresentou boa interação social com os colegas durante a atividade em grupo.",
    },
  ]);
  ...
  const handleEditarAvaliacao = () => {
    ...
    setAvaliacoes(
      avaliacoes.map((av) =>
        av.id === avaliacaoEditando.id
          ? { ...av, data: dataAvaliacao, descricao: descricaoAvaliacao }
          : av
      )
    );
  ...
  const handleExcluirAvaliacao = () => {
    if (!avaliacaoExcluindo) return;

    setAvaliacoes(avaliacoes.filter((av) => av.id !== avaliacaoExcluindo.id));
            ...
            <Dialog open={isAdicionarDialogOpen} onOpenChange={setIsAdicionarDialogOpen}>
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  ...
                  <DialogDescription>Registre uma nova avaliação do aluno</DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                  {/* Data */}
                  <div>
                    <label className="mb-2 block text-[#0D4F97] font-semibold">Data da Avaliação *</label>
                    <Popover>
                  ...
                  {/* Descrição */}
                  <div>
                    <label className="mb-2 block text-[#0D4F97] font-semibold">Descrição da Avaliação *</label>
                    <Textarea
            ...
            <Dialog open={isEditarDialogOpen} onOpenChange={setIsEditarDialogOpen}>
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  ...
                  <DialogDescription>Atualize as informações da avaliação</DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                  {/* Data */}
                  <div>
